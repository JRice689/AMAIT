...
def as_view(request):
    ...
    current_chat_history = DAFI_Question.objects.filter(submitted_by__profile=current_profile)
    ...
    if request.method == "POST":
        ...
        if user_within_limits:
            ...
            found_dafi = find_vector(student_input)

            #Saves the students question BEFORE AI response
            new_question = DAFI_Question()
    ...
    return render(request, 'dafis.html', context)


def get_dafi(request):
    current_user = request.user
    current_profile = Profile.objects.get(user_profile_id=current_user)
    ...
    updated_chat_history = DAFI_Question.objects.filter(submitted_by__profile=current_profile)
    last_question = updated_chat_history.last() 
    found_dafi = getattr(last_question, "from_dafi")
    ...
    full_response = get_openAI_response(found_dafi, short_history, student_input)
...
'''
def get_openAI_response(dafi, history, input):

    prompt = """You are an AI assistant that reads the given 2 page for Air Force regulations to answer
                the users questions. When answer questions you will only use the 2 pages as reference for
                your facts.  Also when you answer you will give the AFI or DAFI number and page number found
                at the top. You will also quote the paragraph number and paragraph you found this information
                from. If you cannot find the information from the given pages inform the user that you cannot
                locate it."""

    messages = [
        {"role": "system", "content": prompt},
        {"role": "system", "content": dafi},
    ]
...
def find_vector(search_text):
    ...
    filePath = Path(settings.BASE_DIR, 'src', 'static', 'dafi.csv')    
...
def get_dafi_chat(request):
    current_user = request.user
    current_profile = Profile.objects.get(user_profile_id=current_user)
    chat_list = list(DAFI_Question.objects.filter(submitted_by__profile=current_profile).values('question', 'response'))
    return JsonResponse({'chat_list': chat_list})